...
class LLMClient:
    ...
    def chat_llm(self, messages: List[Dict[str, str]]) -> Union[Dict, str]:
        """
        Send chat messages to the model and return response.
        Args:
            messages: List of message dicts with "role" and "content" keys
        Returns:
            Local models: vLLM RequestOutput objects -> parsed JSON dict
            OpenAI/Anthropic models: String response content -> parsed JSON dict
        """
        if self.model_type == "local":
            pass
            # return self.model.chat(messages, self.sampling_params)
        elif self.model_type == "openai":
            return self._parse_json_response(self._chat_openai(messages))
        elif self.model_type == "anthropic":
            return self._parse_json_response(self._chat_anthropic(messages))
    ...
    def _parse_json_response(self, response: str) -> Dict:
        ...
        try:
         ...
         # json_response = json.loads(response[response.find("{"): response.rfind("}") + 1]) # from ercp_processor.py
