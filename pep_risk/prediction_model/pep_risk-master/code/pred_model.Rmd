...
```{r clean_data}
# dt_raw = suppressWarnings(read_excel(here("data", "Combined data sheets 11 studies for Analysis June 2020.xlsx"),
                                     # col_types = "numeric"))
# dt_raw = suppressWarnings(read_excel(here("data", "combining data sheets (Elmunzer, Luo, Levenick, Lua, Choi, Buxbaum, Mok, Fogel, Phillip, Lai, Kamal) Sep 2020.xlsx"),
                                     # col_types = "numeric"))
dt_raw = suppressWarnings(read_excel(here("data", "Combined sheet final December.xlsx"), col_types = "numeric"))
...
```{r}
# Create dummy
# dummies <- dummyVars(pep ~ ., data = dt, fullRank = TRUE)
# dt_rest = predict(dummies, newdata = dt) %>% as.data.frame()
# dt = bind_cols(dt_rest, dt %>% select(pep))
...
impute_values = function(train, test, new_dataset = F){
  ...
  # train_impute_step2 <- predict(pre_proc_values, train_impute_step2) # Impute with knn
  # if(!new_dataset){
  #   test_impute_step2 <- predict(pre_proc_values, test_impute_step2)
  # }
  # 
  # train_impute_step2 = train_impute_step2 %>%
  #   bind_rows(., train_impute_step1 %>% filter(!(study_id %in% c(12, 5, 2, 7)))) # Join back to other studies
  # if(!new_dataset){
  #   test_impute_step2 = test_impute_step2 %>%
  #     bind_rows(., test_impute_step1 %>% filter(!(study_id %in% c(12, 5, 2, 7))))
  # }
  # gg_miss_var(train_impute_step2, facet = study_id, show_pct = T) # plot updated missingness
...
varImp(fit)
saveRDS(fit, here("pep_risk_app", "data", "gbm_model.rds"))
# fit = train(pep ~ ., data = train_impute %>% select(-patient_id),
#             method = "rf",
#             trControl = fitControl,
#             verbose = FALSE,
